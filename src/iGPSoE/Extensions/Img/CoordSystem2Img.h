	...
	case KEEP_X_RATIO_COORDSYSTEM2IMG:
		{
			...
			double val = pCS2Img->YIRatio * (pCS2Img->height - 1);
			double temp = (pCS2Img->cs.yMin + pCS2Img->cs.yMax);
	...
	case KEEP_Y_RATIO_COORDSYSTEM2IMG:
		{
			...
			double val = pCS2Img->XJRatio * (pCS2Img->width - 1);
			double temp = (pCS2Img->cs.xMin + pCS2Img->cs.xMax);
