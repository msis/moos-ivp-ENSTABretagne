...
void* REQSRVTCPThrProc(void* lpParam)	{
	...
	struct timeval timeout = {0, 100000}; // Timeout of 100 ms for select()
...
int InitSRVTCP(SRVTCP* pSRVTCP, char* address, char* port)	{
	...
	if (InitTCP(&pSRVTCP->ListenSocket, pSRVTCP->address, pSRVTCP->port, &pSRVTCP->addrinf) != EXIT_SUCCESS)	{
	...
	if (FD_INIT(pSRVTCP->ListenSocket, &pSRVTCP->sock_list, &pSRVTCP->sock_set) != EXIT_SUCCESS)	{
		DelCriticalSection(&pSRVTCP->CSNbConnections);
		DelCriticalSection(&pSRVTCP->CSEndThread);
		StopTCP(&pSRVTCP->ListenSocket, &pSRVTCP->addrinf);
		free(pSRVTCP->address);pSRVTCP->address = NULL;
		free(pSRVTCP->port);pSRVTCP->port = NULL;
