...
void box::Intersect(const box& Y)
{  box X=*this;
   box Z=Inter(X,Y);
   *this=Z;
}
...
{  reel dim=X.dim+Y.dim; box Ans((int)dim);
...
{ int kmax=1;
  reel widthmax=Width(X[kmax])*fabs(Y[kmax].inf);
  for (int k=2; k<=Size(X); k++)
...
//----------------------------------------------------------------------
void BisectHere(box& X, box& X1, box& X2, int i, reel here) //Used by the 3-B consistency
{	X1=X2=X;
	X1[i].sup=here;
	X2[i].inf=here;
}
...
//------------------------------------------------------------------------------
reel decrease(const box& X, const box& Y)
{    reel e=0;
     for (int k=1; k<=X.dim; k++)
	 {  if ((X[k].isEmpty)||(Y[k].isEmpty)) return (-1);
            reel e1=0;
            reel Xinf=X[k].inf, Xsup=X[k].sup;
            reel Yinf=Y[k].inf, Ysup=Y[k].sup;
...
//----------------------------------------------------------------------
reel decrease(const box& X, const box& Y, vector<int> Tab)
{    reel e=0; int size=(int)(Tab.size()-1);
     for (int k=1; k<=size; k++)
	 {  if ((X[Tab[k]].isEmpty)||(Y[Tab[k]].isEmpty)) return (-1);
            reel e1=0;
            reel Xinf=X[Tab[k]].inf, Xsup=X[Tab[k]].sup;
            reel Yinf=Y[Tab[k]].inf, Ysup=Y[Tab[k]].sup;
            if (Xsup>=Ysup) e1=max(e1,Xsup-Ysup);
            if (Xinf<=Yinf) e1=max(e1,Yinf-Xinf);
            if (Xsup<Ysup) e1=max(e1,Ysup-Xsup);
            if (Xinf>Yinf) e1=max(e1,Xinf-Yinf);
...
//------------------------------------------------------------------------------
reel EloignementRelatif2(box& X,box& Y)
{// prend le point X1 de X qui est le plus eloigne de [Y] et renvoie la
        // distance de X1 avec [Y]
             Update(X); Update(Y);
             if (Subset(X,Y)) return 0;
...
//----------------------------------------------------------------------
bool Subset(box& X, box& Y,reel epsilon)
...
//----------------------------------------------------------------------
reel Width(box& X, vector<int>& v)  {
return Width(X[AxePrincipal(X,v)]);}
//----------------------------------------------------------------------
reel Width(box& X, box& Y)
{ int i=AxePrincipal(X,Y);
  return (Width(X[i])*fabs(Y[i].inf));	}
//----------------------------------------------------------------------
reel Marge(box X, box Y)
{  if ((X.IsEmpty())||(Y.IsEmpty())) return -oo;
   reel ans=Marge(X[1],Y[1]);
