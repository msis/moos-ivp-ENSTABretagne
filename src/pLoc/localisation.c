...
#define MESURE // permet d'enregistrer les données dans une fichier
...
void loc_hough(float *rho, float *tt, int size, PARAM *param, POS *pos, float cap){
	...
	if(sin(tt[1]-tt[0])>0){
		for( i = 0; i < size; i++){
			...
			y[i] = sin( tt[i]) * rho[i];
#ifdef MESURE
		fprintf( f, "%f,%f,%f,%f, %f\n", tt[i], rho[i], x[i], y[i], cap);
#endif
		}
	}
	else{
		for( i = 0; i < size; i++){
			x[i] = cos( tt[size-1-i]) * rho[size-1-i];
			y[i] = sin( tt[size-1-i]) * rho[size-1-i];
#ifdef MESURE
		fprintf( f, "%f,%f,%f,%f, %f\n", tt[i], rho[i], x[i], y[i], cap);
#endif
		}
	...
	for( i = 0; i < j; i ++){
		...
		else if(cos(theta[i] - cap - pi/2) > 0.9 && sin(theta[i] - cap -pi/2) < 0.1){		
