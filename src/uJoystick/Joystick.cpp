...
bool Joystick::OnStartUp()
{
  ...
  if(m_MissionReader.GetConfiguration(GetAppName(), sParams))
  {
	...
	if (button_id > 0)
	{
	  ...
	  axis_gain[make_pair(JS_EVENT_BUTTON,button_id)]   = axis_max - axis_min;
	  axis_offset[make_pair(JS_EVENT_BUTTON,button_id)] = axis_min;
	}
	else
...
//         = x(m-m)/65k +0.5(max+min)
	{
	  ...
	  axis_gain[make_pair(JS_EVENT_AXIS,axis_id)]   = (axis_max - axis_min) / 65536.;
	  axis_offset[make_pair(JS_EVENT_AXIS,axis_id)] = (axis_max + axis_min) * 0.5;
	}
