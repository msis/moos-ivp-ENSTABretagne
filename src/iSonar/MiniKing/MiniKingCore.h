...
#ifndef MINIKINGCORE_H
#define MINIKINGCORE_H

#include "OSRS232Port.h"

/*
Debug macros specific to MiniKingUtils.
*/
#ifdef _DEBUG_MESSAGES
#	define _DEBUG_MESSAGES_MINIKINGUTILS
#endif // _DEBUG_MESSAGES

#ifdef _DEBUG_WARNINGS
#	define _DEBUG_WARNINGS_MINIKINGUTILS
#endif // _DEBUG_WARNINGS

#ifdef _DEBUG_ERRORS
#	define _DEBUG_ERRORS_MINIKINGUTILS
#endif // _DEBUG_ERRORS

#ifdef _DEBUG_MESSAGES_MINIKINGUTILS
#	define PRINT_DEBUG_MESSAGE_MINIKINGUTILS(params) PRINT_DEBUG_MESSAGE(params)
#else
#	define PRINT_DEBUG_MESSAGE_MINIKINGUTILS(params)
#endif // _DEBUG_MESSAGES_MINIKINGUTILS

#ifdef _DEBUG_WARNINGS_MINIKINGUTILS
#	define PRINT_DEBUG_WARNING_MINIKINGUTILS(params) PRINT_DEBUG_WARNING(params)
#else
#	define PRINT_DEBUG_WARNING_MINIKINGUTILS(params)
#endif // _DEBUG_WARNINGS_MINIKINGUTILS

#ifdef _DEBUG_ERRORS_MINIKINGUTILS
#	define PRINT_DEBUG_ERROR_MINIKINGUTILS(params) PRINT_DEBUG_ERROR(params)
#else
#	define PRINT_DEBUG_ERROR_MINIKINGUTILS(params)
#endif // _DEBUG_ERRORS_MINIKINGUTILS

/*
Debug macros specific to MiniKingCore.
*/
#ifdef _DEBUG_MESSAGES_MINIKINGUTILS
#	define _DEBUG_MESSAGES_MINIKINGCORE
#endif // _DEBUG_MESSAGES_MINIKINGUTILS

#ifdef _DEBUG_WARNINGS_MINIKINGUTILS
#	define _DEBUG_WARNINGS_MINIKINGCORE
#endif // _DEBUG_WARNINGS_MINIKINGUTILS

#ifdef _DEBUG_ERRORS_MINIKINGUTILS
#	define _DEBUG_ERRORS_MINIKINGCORE
#endif // _DEBUG_ERRORS_MINIKINGUTILS

#ifdef _DEBUG_MESSAGES_MINIKINGCORE
#	define PRINT_DEBUG_MESSAGE_MINIKINGCORE(params) PRINT_DEBUG_MESSAGE(params)
#else
#	define PRINT_DEBUG_MESSAGE_MINIKINGCORE(params)
#endif // _DEBUG_MESSAGES_MINIKINGCORE

#ifdef _DEBUG_WARNINGS_MINIKINGCORE
#	define PRINT_DEBUG_WARNING_MINIKINGCORE(params) PRINT_DEBUG_WARNING(params)
#else
#	define PRINT_DEBUG_WARNING_MINIKINGCORE(params)
#endif // _DEBUG_WARNINGS_MINIKINGCORE

#ifdef _DEBUG_ERRORS_MINIKINGCORE
#	define PRINT_DEBUG_ERROR_MINIKINGCORE(params) PRINT_DEBUG_ERROR(params)
#else
#	define PRINT_DEBUG_ERROR_MINIKINGCORE(params)
#endif // _DEBUG_ERRORS_MINIKINGCORE

// Motor Step Angle Size : scanning motor step angle between pings in 1/16 Gradian units
// Low Resolution = 32 (= 2 Gradians = 1.8 °)
#define LOW_RESOLUTION_MINIKING 32
// Medium Resolution = 16 (= 1 Gradian = 0.9°)
#define MEDIUM_RESOLUTION_MINIKING 16
// Mgr Resolution = 8 (= 0.5 Gradian = 0.45°)
#define MGR_RESOLUTION_MINIKING 8
// Ultimate Resolution = 4 (= 0.25 Gradian = 0.225°)
#define ULTIMATE_RESOLUTION_MINIKING 4
...
{
	int minBin = my_max(0, (int)(minDist*(double)NBins/(double)RangeScale));
	int maxBin = my_min(NBins, (int)(maxDist*(double)NBins/(double)RangeScale));
	int i = 0;
...
/*
Read a message from a MiniKing.
...
uint8 cmd_byte : (IN) Message number (see the MiniKing documentation).

Return : EXIT_SUCCESS or EXIT_FAILURE if there is an error.
*/
inline int ReadMsgMiniKing(HANDLE hDev, uint8* readbuf, UINT nb_bytes, uint8 cmd_byte)
{
	UINT j = 0;
	UINT BytesRead = 0;
	...
	if (nb_bytes <= 10)
	{
		PRINT_DEBUG_ERROR_MINIKINGCORE(("ReadMsgMiniKing error (%s) : %s"
	...
	{
		PRINT_DEBUG_WARNING_MINIKINGCORE(("ReadMsgMiniKing warning (%s) : %s"
			"(hDev=%#x, nb_bytes=%u, cmd_byte=%#x)\n", 
			strtime_m(), 
...
/*
Wait for a mtAlive message from a MiniKing.

HANDLE hDev : (IN) Identifier of the device serial port.

Return : EXIT_SUCCESS or EXIT_FAILURE if there is an error.
*/
inline int WaitForAliveMsgMiniKing(HANDLE hDev)
{
	uint8 readbuf[22];
	...
	{ 
		PRINT_DEBUG_ERROR_MINIKINGCORE(("WaitForAliveMsgMiniKing error (%s) : %s"
			"(hDev=%#x)\n", 
			strtime_m(), 
...
/*
Get the version of a MiniKing. This message must be sent to be able to use the 
...
Return : EXIT_SUCCESS or EXIT_FAILURE if there is an error.
*/
inline int GetVersionMiniKing(HANDLE hDev)
{
	uint8 readbuf[25];
	...
	{ 
		PRINT_DEBUG_ERROR_MINIKINGCORE(("GetVersionMiniKing error (%s) : %s"
			"(hDev=%#x)\n", 
			strtime_m(), 
	...
	if (ReadMsgMiniKing(hDev, readbuf, 25, 0x01) != EXIT_SUCCESS)
	{ 
		PRINT_DEBUG_ERROR_MINIKINGCORE(("GetVersionMiniKing error (%s) : %s"
			"(hDev=%#x)\n", 
			strtime_m(), 
...
/*
Get various settings of a MiniKing. This message must sometimes be sent to be 
...
Return : EXIT_SUCCESS or EXIT_FAILURE if there is an error.
*/
inline int GetUserMiniKing(HANDLE hDev)
{
	uint8 readbuf[264];
	...
	if (WriteAllRS232Port(hDev, writebuf, 14) != EXIT_SUCCESS)
	{ 
		PRINT_DEBUG_ERROR_MINIKINGCORE(("GetUserMiniKing error (%s) : %s"
			"(hDev=%#x)\n", 
			strtime_m(), 
			"Error writing data to the serial port. ", 
			hDev));
	...
	if (ReadMsgMiniKing(hDev, readbuf, 264, 0x06) != EXIT_SUCCESS)
	{ 
		PRINT_DEBUG_ERROR_MINIKINGCORE(("GetUserMiniKing error (%s) : %s"
			"(hDev=%#x)\n", 
			strtime_m(), 
...
/*
Reboot a MiniKing. This should take several seconds. After it is rebooted, 
...
Return : EXIT_SUCCESS or EXIT_FAILURE if there is an error.
*/
inline int RebootMiniKing(HANDLE hDev)
{
	uint8 readbuf[22];
	...
	if (WriteAllRS232Port(hDev, writebuf, 14) != EXIT_SUCCESS)
	{ 
		PRINT_DEBUG_ERROR_MINIKINGCORE(("RebootMiniKing error (%s) : %s"
			"(hDev=%#x)\n", 
			strtime_m(), 
			"Error writing data to the serial port. ", 
			hDev));
...
/*
Set various parameters in a MiniKing. This message is important to be able to 
...
Return : EXIT_SUCCESS or EXIT_FAILURE if there is an error.
*/
inline int SetParamMiniKing(HANDLE hDev, SNRPARAMS params)
{
	struct HdCtrlStruct
	{
...
/*
Request scanlines from a MiniKing. Use SendDataReplyMiniKing() to get the 
reply.
...
Return : EXIT_SUCCESS or EXIT_FAILURE if there is an error.
*/
inline int SendDataRequestMiniKing(HANDLE hDev)
{
	time_t t;
	uLong dword;
	...
	{ 
		PRINT_DEBUG_ERROR_MINIKINGCORE(("SendDataRequestMiniKing error (%s) : %s"
			"(hDev=%#x)\n", 
			strtime_m(), 
			"Error writing data to the serial port. ", 
			hDev));
...
Use it after a call to SendDataRequestMiniKing().
...
Return : EXIT_SUCCESS or EXIT_FAILURE if there is an error.
*/
inline int SendDataReplyMiniKing(HANDLE hDev, SNRPARAMS params,
							 ...
							 uint8* scanline2, double* pAngle2)
{
	...
	if (ReadMsgMiniKing(hDev, readbuf1, Dbytes+45, (uint8)0x02) != EXIT_SUCCESS)
	{ 
		PRINT_DEBUG_ERROR_MINIKINGCORE(("SendDataReplyMiniKing error (%s) : %s"
			"(hDev=%#x)\n", 
			strtime_m(), 
			"The device is not responding correctly. ", 
			hDev));
	...
	if (ReadMsgMiniKing(hDev, readbuf2, Dbytes+45, (uint8)0x02) != EXIT_SUCCESS)
	{ 
		PRINT_DEBUG_ERROR_MINIKINGCORE(("SendDataReplyMiniKing error (%s) : %s"
			"(hDev=%#x)\n", 
			strtime_m(), 
